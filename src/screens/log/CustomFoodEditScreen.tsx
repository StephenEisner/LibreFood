...
import { getCustomFoodById, updateCustomFood } from '../../services/database/foods';
import type { NutritionData } from '../../types/foods';

type Nav = NativeStackNavigationProp<LogStackParamList, 'CustomFoodEdit'>;
type RouteT = RouteProp<LogStackParamList, 'CustomFoodEdit'>;
...
export function CustomFoodEditScreen() {
  ...
  useEffect(() => {
    getCustomFoodById(id).then((food) => {
      if (!food) {
      ...
      setServingSizeUnit(food.serving_size_unit);
      setHouseholdText(food.household_serving_text ?? '');
      ...
      const loadedMicros: Record<string, string> = {};
      for (const field of MICRO_FIELDS) {
        const val = nutrition[field.key];
        if (val != null) {
          loadedMicros[field.key] = String(val);
        }
      }
      if (Object.keys(loadedMicros).length > 0) {
    ...
    >
      ...
      >
        ...
        <View style={styles.card}>
          <LabeledInput label="Food Name *" value={name} onChangeText={setName} placeholder="e.g. Chicken Breast" />
          <LabeledInput label="Brand (optional)" value={brand} onChangeText={setBrand} placeholder="e.g. Generic" />
        </View>
        ...
        <View style={styles.card}>
          <View style={styles.servingRow}>
            <View style={{ flex: 1, marginRight: spacing.sm }}>
              <LabeledInput label="Amount" value={servingSize} onChangeText={setServingSize} keyboardType="decimal-pad" placeholder="100" />
            </View>
            <View style={{ flex: 1 }}>
              <LabeledInput label="Unit" value={servingSizeUnit} onChangeText={setServingSizeUnit} placeholder="g" />
            </View>
          </View>
          <LabeledInput label="Household Measure (optional)" value={householdText} onChangeText={setHouseholdText} placeholder="e.g. 1 cup" />
        ...
        <Pressable style={styles.microsToggle} onPress={() => setShowMicros((v) => !v)}>
